        ...
        if ($request->isMethod('POST') ) {
            ...
            if(!empty($dataSet['results'][0])){
                ...
                if(!empty($medicationRxcuis)) {
                    ...
                    $commonTestsLisAndDb = DB::select("SELECT test_details.test_name, test_details.class, test_details.description, test_details.LLOQ,test_details.LLOQ, ULOQ, lis_orders_test_results.result_quantitative, lis_orders_test_results.result_qualitative FROM test_details INNER JOIN lis_orders_test_results ON test_details.test_name = lis_orders_test_results.test_name WHERE lis_orders_test_results.order_code='".$orderCode."'");
                    ...
                    if(!empty($commonTestsLisAndDb)){
                        foreach ($commonTestsLisAndDb as $commonTestsLisAndDb_key => $commonTestsLisAndDb_value) {
                            if (in_array($commonTestsLisAndDb_value->test_name, $prescribedMedicineNameArray) || in_array($commonTestsLisAndDb_value->test_name, $metaboliteNameArray)){
                                ...
                                if($commonTestsLisAndDb_value->description){
                                    ...
                                    $metaboliteOfThisParent_query = Lis_medication_details::where(['order_code' => $orderCode, 'medication_name' => $commonTestsLisAndDb_value->test_name])->get(['metabolite_name']);
                                ...
                                if(in_array($parentOfThisMetabolite, $detectedMedicines) || in_array($commonTestsLisAndDb_value->test_name, $detectedMedicines) || count(array_intersect($metaboliteOfThisParent, $detectedMedicines)) > 0){

                                    $prescribedDetected[$commonTestsLisAndDb_value->test_name] = array('class' => $commonTestsLisAndDb_value->class, 'description' => $commonTestsLisAndDb_value->description, 'LLOQ' =>  $commonTestsLisAndDb_value->LLOQ, 'ULOQ' => $commonTestsLisAndDb_value->ULOQ, 'result_quantitative' => $commonTestsLisAndDb_value->result_quantitative, 'result_qualitative' => $commonTestsLisAndDb_value->result_qualitative);
                                }
                                else{

                                    $prescribedNotDetected[$commonTestsLisAndDb_value->test_name] = array('class' => $commonTestsLisAndDb_value->class, 'description' => $commonTestsLisAndDb_value->description, 'LLOQ' =>  $commonTestsLisAndDb_value->LLOQ, 'ULOQ' => $commonTestsLisAndDb_value->ULOQ, 'result_quantitative' => $commonTestsLisAndDb_value->result_quantitative, 'result_qualitative' => $commonTestsLisAndDb_value->result_qualitative);
                                }
                            }
                            else{
                    ...
                    if(isset($tests) && !empty($tests)){
                        foreach($tests as $resultValue){
                            foreach($allSectionTest as $key => $valueTestName){
                                if(ucwords($valueTestName) == ucwords($resultValue->test_name)|| $valueTestName == $resultValue->test_name){
                    ...
                    foreach($arrayResult as $testName => $ciDescription){
                        ...
                        foreach($tests as $resultValue){
                            ...
                            if(ucwords($testName) == ucwords($resultValue->test_name)){
