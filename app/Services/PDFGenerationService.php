...
use App\Services\GetDatasetService;
...
class PDFGenerationService
{
    ...
    protected GetDatasetService $_getDatasetService;
    protected DBService $_DBservice;
    ...
    public function __construct()
    {
        ...
        $this-> _getDatasetService = new GetDatasetService();
    ...
    public function getOutPutData_From_mock()
    {
        $filePath = storage_path('./../MockData/mock_output.json');
        $jsonData = File::get($filePath);
        $dataSet = json_decode($jsonData, true);
    ...
    public function getPdfDataForClin1($orderCode, $show_log = 0)
    {
        ...
        try 
        {
            ...
            {
                // 2. get from Database by orderCode
                $this->dump_array('Get dataset from DB: ', []);
                $show_log == 1;
                try
                {
                ...
                {
                    Log::channel('error')->error('Inser into DB : Dataset: Exception: ' . $ex->getMessage()); 
                    $this->dump_array('Inser into DB : Dataset: Exception: ' , $ex->getMessage());
                }
            }
            ...
            if (!empty($dataSet)) 
            {
                ...
                if (!blank($icdCodeArray) && !empty($icdCodeArray) && filled($icdCodeArray) &&  is_array($icdCodeArray) && count($icdCodeArray) > 0) 
                {
                    ...
                    Log::channel('error')->error('ICD codes not available, orderCode' . $orderCode);
                ...
                if (true) 
                {
                            ...
                            foreach ($meshConditions as $keys => $values) 
                            {
                                foreach ($values as $key => $value) // TODO: $key is used as a key in both inner and outer loop. Change name?
                                {
                                    if (in_array(strtolower($value), $prescribedMedications)) {
                                        ...
                                        $this->dump_array("arrayResult_CI_Data_On_icdMesh___111", $arrayResult_CI_Data_On_icdMesh);
                                        $this->dump_array("icdCodes", $icdCodes);

                                        // foreach ($icdCodes as $key => $valueCode) {
                                            foreach ($icdCodes as $valueCode) {
                                             $this->dump_array("called getMeshCode :", $valueCode);
                                            $meshCodeData = $this->getMeshCode($valueCode);
                                            $this->dump_array("get mesh code data :", $meshCodeData);
                        ...
                        $this->Log_scheduler_info('-> medicationRxcuis___array_1__after_unique: ' . json_encode($medicationRxcuis));
                        $this->Log_scheduler_info('-> rxcuis_string: ' . $rxcuis_string);
                        ...
                        if (!blank($ddi_row_data) && !empty($ddi_row_data) && filled($ddi_row_data) &&  is_array($ddi_row_data) && count($ddi_row_data) > 0) 
                        {
                            ...
                            foreach ($type as $key => $value) {
                                 ...
                                 */
                                $testDetail = $tests_table_test_details->where('test_name', ucwords($drugName));
                                $drugClassName = '';
                                ...
                                //         if($result->test_name == ucwords($prescribedTestName->prescribed_test)){
                ...
                {
                    Log::channel('error')->error('No data found (NO -ddi_row_data), orderCode : '. $orderCode);
                    ...
                    OrderDetail::where('order_code', $orderCode)
                        ...
                        $this->dump_array("insert into db  :: arrayResult_CI_Data_On_icdMesh ", $arrayResult_CI_Data_On_icdMesh);
                    ...
                    if (str_contains($testInformation, 'Clin1') || true) // TODO: Added this true for Stratus. Need to remove.
                    {
                        if (str_contains($testInformation, '4 Panel Urine Screen') || true) { // TODO: '4 Panel Urine Screen' not found in Panel Name
                            ...
                            $panelTestsArray = [
                                ...
                                // 'Creatinine',  // special case
                                'Oxidant Scr',
                                // 'Specific Gravity',   // special case
                                'Synthetic Urine',
                                // 'Hydroxyalprazolam',
                                // 'Alprazolam'
                            ];
                            ...
                            foreach ($testResults as $testResult) {
                                if (in_array($testResult['test_description'], $panelTestsArray)) {
                                    ...
                                    $panelTestResult[$panelTestName]['test_cutoffvalue'] = $testResult['test_cutoffvalue'];
                                    $panelTestResult[$panelTestName]['result_display'] = $testResult['result_display'];
                                }
                            }
                        }
                        ...
                        foreach ($testResults as $key => $value) {
                            ...
                            if (in_array($value['test_description'], $testTypeArray)) {
                                if ($value['test_description'] == 'pH') {
                                    ...
                                    $quantitativeResultSpecificGravity =  $quantitativeResultSpecificGravitys; // round($quantitativeResultSpecificGravitys, 3);
                                } elseif (
                        ...
                        foreach ($detectedPrescribed as $value) {
                            ...
                            foreach ($tests_table_test_details as $test) {
                                if (strtolower($test->test_name) == strtolower($value["test_type"])) {
                        ...
                        foreach($prescribedTestDetected as $testId => $testResult) {
                            foreach($testResult as $testName => $testData) {
                                foreach($tests_table_test_details as $resultValue) {
                                    if(ucwords($resultValue->test_name) == $testName
                                        || $resultValue->test_name == $testName) {
                        ...
                        */
                        ...
                        foreach ($detectedMedicines as $index => $value) {
                            if (!empty($medicineNamesArray)) {
                                if (
                                    ...
                                    foreach ($tests_table_test_details as $resultValue) {
                                        if (ucwords($resultValue->test_name) == ucwords($index)) {
                        ...
                        $chunkSize = intval(($collection->count())/4);
                        $this->dump_array("chunkSize", $chunkSize);
                        $this->dump_array("collection->count", $collection->count());
                        

                        $chunks = $collection->chunk($chunkSize+1); // $collection->chunk(12);
                        ...
                        // // Show ULOQ - value decision
                        // foreach($sortedNotPrescribedDetected as $sortedNotPrescribedDetected_single)
                        // {
                        // }
                        ...
                        $data = [
                            ...
                            // 'notDetectedNotPrescribed_array' => $notDetectedNotPrescribed_new_variable,
                            ...
                            'orders' => $orders,
                            'receivedDate' => $received_date_formated,  // date("m/d/Y", strtotime($receivedDate)),
                            'labLocations' => $labLocations,  // $labLocations_Stratus, // $labLocations,
                            'labLocations_Stratus' => $labLocations_Stratus,
    ...
    public function uploadToS3($fileName, $file_content)
    {
        // $s3 = new S3Client([
        //     'version' => 'latest',
        ...
        try {
            $result = $s3->putObject([
                'Bucket' => $bucket,
                'Key' => $filePath,
                'Body' => $file_content, // fopen(public_path() . '/pdf//' . $fileName, 'r'),
    ...
    public function postPDF_to_Stratus($fileContents, $orderCode)
    {
       ...
       // $fileContents = file_get_contents($pdfFilePath);
        ...
        $requestEndpoint = Config::get('nih.stratus_post_base64_report_pdf_api');

        $accession_id =   explode("-", $orderCode)[0];
        $this->Log_scheduler_info('-> accession_id: ' . $accession_id);
        $reqestBody = [
            "order" => [
    ...
    public function common_getPDFReport_service($orderCode, $show_log = 0)
    {
        ...
        try 
        {
                ...
                if ($show_log == 1) {
                    dump('called method : getPdfDataForClin1 ');
                }

                $this->Log_scheduler_info('-> pdf generation - call getPdfDataForClin1 for order_Code: ' . $orderCode);
                // dd("sahariar");

                $response = $this->getPdfDataForClin1($orderCode, $show_log);
                // dd($response,"sahariar");
                ...
                $this->Log_scheduler_info('got pdf data: ');
                ...
                if (env('UPLOAD_PDF_TO_S3') == 'yes') {
                    if ($this->uploadToS3($fileName, $pdf->output())) {
    ...
    ///Test Purpose
    public function test_generatePDF_from_mock_output()
    {
        ini_set('max_execution_time', '0');
        $show_log = 1;
        ...
        try 
        {
                ...
                $this->get_env_TRY_IF_FAILED_TIMES();

                $data = $this->getOutPutData_From_mock();
                // $data = $this-> arrayToObject( $data);
                dump('mock data output :: ');
                dump('called method : test_generatePDF_from_mock_output');
                dump($data);

                // 1. Location
                // $counter = 0;
                // foreach($data['labLocations'] as $location)
                // {
                //     $data['labLocations'][$counter] = (object)$location;
                ...
                // }
                 ...
                 $data['labLocations'] = (object)(collect((object) $data['labLocations']));
                 dump($data);
                ...
                {
                    $data['orders'][$counter] = (object)$order;
                    //$order = (object)$order;
                    // dump($order->order_code);
                ...
                {
                    $data['tests'][$counter] = (object)$test;
                    //$test = (object)$test;
                    //dump($test->test_name);
                ...
                {
                    $data['sections'][$counter] = (object)$section;
                    //$section = (object)$section;
                    //dump($section->section_id);
                ...
                // 5. generalComments
                $counter = 0;
                foreach($data['generalComments'] as $generalComment)
                {
                    $data['generalComments'][$counter] = (object)$generalComment;
                    //$generalComment = (object)$generalComment;
                    //dump($generalComment->id);

                    $counter++;
                }

                // 5. notDetectednotPrescribed
                // $data['notDetectednotPrescribed'] = (object)( $data['notDetectednotPrescribed']);
                
                $counter = 0;
                foreach($data['notDetectednotPrescribed'] as $notDetectednotPrescribed)
                {
                    $data['notDetectednotPrescribed'][$counter] = (object)$notDetectednotPrescribed;
                    //$notDetectednotPrescribed = (object)$notDetectednotPrescribed;
                    //dump($notDetectednotPrescribed);

                    $counter++;
                }

                $data['notDetectednotPrescribed'] = collect( $data['notDetectednotPrescribed']);

                // // 2. Create PDF
                // $pdf = PDF::loadView('generatePdfReport', $data)->setPaper('a1', 'portrait');  
                // $pdf->getDomPDF()->set_option("enable_php", true);
            
                // $fileName = "generate_test.pdf";
                // // $filePath = 'pdf/' . $fileName;
                // $filePath =  $fileName;
                ...
                //         dump($url);
                //     }
                //     dump('pdf uploaded to s3 url: ' . $url);
                // }
            ...
            return $pdf->download('generatePDF_test.pdf');
        } 
        catch (Exception $exx) 
        {
            dump('$$$$ --> Exception->' . $exx->getMessage());
        }
        finally 
        {
            //GC collection
    ...
    public function get_env_TRY_IF_FAILED_TIMES()
    {
        $numbers_str = env('TRY_IF_FAILED_TIMES', '2');
        $numbers = intval($numbers_str);
        return $numbers;
        
    // dump($numbers);
    // $test =  DB::table('order_code_queues')->where('workStatus', '=', 0)->where('numberOfFailur', '<', $numbers)->take(1)->get();
    // dump($test);
    }
